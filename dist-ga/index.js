...
exports.defaultOptions = {
    ...
    emptyCommitMessage: process.env.RAB_EMPTY_COMMIT_MSG || 'chore: automatic empty commit',
    pullRequestTitle: process.env.RAB_PULL_REQUEST_TITLE || 'fix: dependencies',
    pullRequestBody: process.env.RAB_PULL_REQUEST_BODY || 'Weekly dependencies update.',
};
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.resetBranch = exports.createPR = exports.createEmptyCommit = exports.isCommitAnEmptyCommit = exports.getCommit = exports.updateRef = exports.deleteRef = exports.createBranch = exports.getRef = exports.wait = void 0;
...
exports.wait = wait;
async function getRef(octokit, branch, opts) {
    try {
...
async function createBranch(octokit, sha, opts) {
...
async function deleteRef(octokit, opts) {
...
async function updateRef(octokit, sha, opts) {
...
async function getCommit(octokit, sha, opts) {
...
function isCommitAnEmptyCommit(commit, opts) {
    return commit.message.search(opts.emptyCommitMessage) >= 0;
}
exports.isCommitAnEmptyCommit = isCommitAnEmptyCommit;
async function createEmptyCommit(octokit, refCommit, opts) {
...
async function createPR(octokit, opts) {
    // Next monday
    const date = new Date();
    date.setDate(date.getDate() + 3);
    const title = `${opts.pullRequestTitle || 'fix: dependencies'} ${date.toISOString().split('T')[0]}`;
...
async function resetBranch(octokit, refBase, exists, opts) {
    if (exists) {
...
async function run(args) {
    const opts = { ...constants_1.defaultOptions, ...args };
    if (!opts.ghToken) {
    ...
    try {
        ...
        const refBase = await (0, helpers_1.getRef)(octokit, opts.branchBase, opts);
        const refTarget = await (0, helpers_1.getRef)(octokit, opts.branchToCreate, opts);
        ...
        if (refTarget) {
            ...
            const commit = await (0, helpers_1.getCommit)(octokit, refTarget, opts);
            if ((0, helpers_1.isCommitAnEmptyCommit)(commit, opts)) {
                console.log('Empty commit exists');
                return;
            }
        }
        await (0, helpers_1.resetBranch)(octokit, refBase, Boolean(refTarget), opts);
        console.log('Creating pull request');
        await (0, helpers_1.createPR)(octokit, opts);
    }
    catch (err) {
        console.error(err);
